<template>
	<view class="container">
			...
			<view v-for="(item, index) in timelineData" :key="index" class="timeline-item">
				...
				<view class="nature-card" @click="handleDayClick(index)">
					<view class="card-header">
						<text class="day-title">第{{index+1}}次记录</text>
			...
			<view class="edit-panel">
				...
				<view class="panel-body">
					...
					<!-- 					<view style="display: flex;">
						<view v-for="item in fileList"  :key="item.size">
							<img v-if="item.fileType=='image'" :src="item.tempFilePath" alt="">
							<video v-if="item.fileType=='video'" :src="item.tempFilePath" alt=""></video>
						</view>
					</view> -->
		...
		<view
			style="position: fixed; background-color: #55ff7f; padding: 20rpx;border-radius: 50%; text-align: center; display: flex; box-shadow: 1rpx 1rpx 3rpx 2rpx gray; right: 20rpx; bottom: 50rpx;"
			@click="addRecord"><uni-icons type="plusempty" size="40"></uni-icons></view>
	</view>
</template>
